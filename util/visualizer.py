...
class COCOVisualizer():
    ...
    def visualize(self, img, tgt, caption=None, dpi=140, savedir=None, show_in_console=True, score_flag=False):
        """
        img: tensor(3, H, W)
        ...
        self.addtgt(tgt, score_flag=score_flag)
        if show_in_console:
            plt.show()
    ...
    def addtgt(self, tgt, score_flag=False):
        """
        ...
        if 'box_label' in tgt:
            ...
            for idx, bl in enumerate(tgt['box_label']):
                if score_flag:
                    _string = str(bl) + ' probability: ' + str(round(float(tgt['scores'][idx]), 2))
                else:
                    _string = str(bl)
