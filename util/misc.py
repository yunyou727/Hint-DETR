...
def uvcollate_fn(batch):
    # import ipdb; ipdb.set_trace()
    batch = list(zip(*batch))
    batch[0] = nested_tensor_from_tensor_list(batch[0])
    batch[1] = nested_tensor_from_tensor_list1(batch[1])
    return tuple(batch)


def uvcollate_fn_bg(batch):
    # import ipdb; ipdb.set_trace()
    batch = list(zip(*batch))
    batch[0] = nested_tensor_from_tensor_list(batch[0])
    batch[1] = nested_tensor_from_tensor_list1(batch[1])
...
def nested_tensor_from_tensor_list1(tensor_list: List[List[Tensor]]):
    # TODO make this more general
    num_class = 6
    if len(tensor_list[0]) == num_class:
        ...
        for i in range(0, len(tensor_list)):
            ...
            for j in range(0, len(x)):
                if torch.is_tensor(x[j]):
                    l1.append(list(x[j].shape))
                    dtype = x[j].dtype
                    device = x[j].device

        # TODO make it support different-sized images
        # max_size = _max_by_axis([list(img.shape) for img in tensor_list])
        max_size = _max_by_axis(l1)
        # min_size = tuple(min(s) for s in zip(*[img.shape for img in tensor_list]))
        batch_shape = [len(tensor_list)] + [len(tensor_list[0])] + max_size
        b, num, c, h, w = batch_shape
        ...
        for i in range(0, len(tensor_list)):
            ...
            for j in range(0, len(x)):
                if torch.is_tensor(x[j]):
                    tensor = torch.zeros((c, h, w), dtype=dtype, device=device)
                    mask = torch.ones((h, w), dtype=torch.bool, device=device)
                    """
                    for l1 in range(0,x[j].shape[0]):
                        for l2 in range(0,x[j].shape[1]):
                            for l3 in range(0,x[j].shape[2]):
                                tensor[l1,l2,l3]=(x[j])[l1,l2,l3]
                    """
                    # tensor[ : x[j].shape[0], : x[j].shape[1], : x[j].shape[2]].copy_(x[j])
                    tensor[: x[j].shape[0], : x[j].shape[1], : x[j].shape[2]] = (x[j])
                    mask[: x[j].shape[0], :x[j].shape[1]] = False
                    x[j] = NestedTensor(tensor, mask)
