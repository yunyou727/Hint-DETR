...
from datasets import uvbuild_dataset
from engine import evaluate, train_one_epoch
from models.Hint_DETR.DETR import build_DETR
from util.utils import clean_state_dict


def get_args_parser():
    parser = argparse.ArgumentParser('Hint-DETR', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--modelname', '-m', type=str, required=True, choices=['hint_detr'])
    ...
    parser.add_argument('--cosine_loss', default=8, type=float,
                        help="cosine_loss for background")
    ...
    parser.add_argument('--pretrain_model_path', help='load from checkpoint')
    ...
    parser.add_argument('--pretrain_new', help='load from the pre-training model of mutil_scale')
    parser.add_argument('--eval_every_epoch', default=1, type=int, help='eval every ? epoch')
    return parser


def build_model_main(args):
    if args.modelname.lower() == 'hint_detr':
...
def main(args):
    ...
    dataset_train = uvbuild_dataset(image_set='train', args=args)
    ...
    data_loader_train = DataLoader(dataset_train, batch_sampler=batch_sampler_train,
                                   collate_fn=utils.uvcollate_fn_bg, num_workers=args.num_workers)
    ...
    if not args.resume and args.pretrain_model_path:
        ...
        _tmp_st = OrderedDict(
            {k: v for k, v in clean_state_dict(state_dict).items() if check_keep(k, _ignorekeywordlist)})
        _load_output = model_without_ddp.load_state_dict(_tmp_st, strict=False)
        logger.info(str(_load_output))
        # import ipdb; ipdb.set_trace()
        if args.pretrain_new:
            print('Initialized from the pre-training model of mutil_scale')
            checkpoint1 = torch.load(args.pretrain_new, map_location='cpu')

            state_dict1 = checkpoint1['model']
            for k in list(state_dict1.keys()):
                # remove useless class embed
                if 'input_proj' not in k \
                        and 'level_embed' not in k:
                    del state_dict1[k]
            for k in list(state_dict1.keys()):
                print(k)
                match = re.search('input_proj', k)
                if match:
                    new_k = re.sub('input_proj', 'input_proj_mutil', k)
                    state_dict1[new_k] = state_dict1[k]
                    del state_dict1[k]
            # for k in list(state_dict1.keys()):
            #     print(k)
            _load_output = model_without_ddp.load_state_dict(state_dict1, strict=False)
            logger.info(str(_load_output))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_every_epoch == 0: #or epoch>36:
